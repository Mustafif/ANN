...
def calibration_HN_GARCH(assets, options_data, model=hn_garch_model, batch_size=2048, seed=None, strategy='rand1bin', bounds=bounds, polish=True):
        ...
        def objective_fn(x, x0=x0, lr=log_returns, S0=S0, m=m, r=r, T=T, corp=corp, V=V, sigma_obs=sigma_obs):
            sigma_eps = 0.01
            ...
            for i in range(lr_size-1):
                h[i+1] = omega + beta*h[i] + alpha*((lr[i]-r-lambda_*h[i])/torch.sqrt(h[i]) - gamma*torch.sqrt(h[i]))**2
            ...
            Y2 = -0.5 * torch.sum(
                2 * torch.log(sigma_eps)
                + ((sigma_obs - sigma_model) / sigma_eps) ** 2
            )
        ...
        # init_pop[0] = x0
        kwargs = dict(args=(),strategy=strategy, maxiter=200,
                                popsize=15, tol=1e-2, mutation=(0.5, 1),
                                recombination=0.8, seed=seed, callback=None,
                                disp=True, polish=polish, init='sobol', x0=x0)
        t0 = time.time()
